...
ini=unique(f[argmax(f[2:end,2])+1,1])
println(ini)


for j in collect(1:1:17)
    ...
    println(length(unique(seq_reached)))
...
writedlm("growing_alphabets2_mean_var_$(index)_RAW=$(RAW)_land=$(land).txt",rrr)
